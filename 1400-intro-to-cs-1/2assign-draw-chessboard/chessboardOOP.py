class Chessboard:
    ...
    def __drawAllRectangles(self):
        self.__smallW = self.__width / 8
        self.__smallH = self.__height / 8

        self.__tr.pu()
        self.__tr.home()
        self.__tr.setpos(self.__startX, self.__startY - self.__smallH * 8)
        for i in range(0, 4):
        ...
        for x in range(2):
            ...
            for h in range(4):
                ...
                for w in range(4):
                    ...
                    self.__tr.fd(self.__smallW)
                    self.__tr.left(90)
                    self.__tr.fd(self.__smallH)
                    self.__tr.left(90)
                    self.__tr.fd(self.__smallW)
                    self.__tr.left(90)
                    self.__tr.fd(self.__smallH)
                    self.__tr.left(90)
                    self.__tr.end_fill()
                    self.__tr.fd(self.__smallW * 2)
                self.__tr.seth(90)
                self.__tr.fd(self.__smallH * 2)
                self.__tr.seth(180)
                self.__tr.fd(self.__width)
            self.__tr.setpos(self.__startX + self.__smallW, self.__startY - self.__smallH * 7)
